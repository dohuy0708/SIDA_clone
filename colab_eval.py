...
def main():
    ...
    # Prompt cố định cho mọi bức ảnh
    ...
    for image_path in image_paths:
        ...
        csv_path = os.path.join(args.vis_save_path, "results.csv")
        write_header = not os.path.exists(csv_path)
        with open(csv_path, "a") as f:
            if write_header:
                f.write("filename,classification,has_mask\n")
            has_mask = len(pred_masks) > 0
            f.write(f"{os.path.basename(image_path)},{cls_label},{has_mask}\n")

        if len(pred_masks) > 0:
            # Ảnh bị tampered → lưu mask thật
            for i, pred_mask in enumerate(pred_masks):
                ...
                save_path = "{}/{}_mask_{}.jpg".format(
                    args.vis_save_path, base_name, i
                )
            ...
            # Ảnh real hoặc synthetic → lưu mask trống (đen) + ảnh gốc
            h, w = image_np.shape[:2]
            blank_mask = np.zeros((h, w), dtype=np.uint8)

            save_path = "{}/{}_mask_0.jpg".format(args.vis_save_path, base_name)
            cv2.imwrite(save_path, blank_mask)

            save_path = "{}/{}_masked_img_0.jpg".format(args.vis_save_path, base_name)
